...
template <typename ComponentType> class Vector
{
public:
    // Default-constructor.
    Vector();
    ...
    explicit Vector(Tensor<ComponentType> &&tensor);
...
template <typename ComponentType> class Matrix
{
public:
    // Default-constructor.
    Matrix();
    ...
    // construction from tensor
    explicit Matrix(Tensor<ComponentType> &&tensor);
...
template <typename ComponentType> Vector<ComponentType>::Vector(const std::string &filename)
{
...
template <typename ComponentType> Matrix<ComponentType>::Matrix(const std::string &filename)
{
    tensor_ = readTensorFromFile<ComponentType>(filename);
    if (tensor_.rank() != 2)
    {
...
template <typename ComponentType> size_t Matrix<ComponentType>::rows() const
{
    return tensor_.shape()[0];
}

template <typename ComponentType> size_t Matrix<ComponentType>::cols() const
{
